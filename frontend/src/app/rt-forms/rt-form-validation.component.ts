...
export abstract class RtFormValidationComponent<T> implements OnInit, OnDestroy {
  ...
  ngOnInit(shouldInitForm = true): void {
    ...
    // Otherwise, it should be initialized in a child component (added for cases when manual init required).
    if (shouldInitForm) {
  ...
  /**
   * Generate a form group using provided fields data.
   */
  protected initializeFormGroup(formFields: RtFormFields<Partial<T>>): FormGroup<TransformToTypedForm<T>> {
    const model: any = {};
    for (const name of Object.keys(formFields)) {
      if (formFields[name as keyof T]) {
        model[name] = ['', formFields[name as keyof T]];
      } else {
        model[name] = '';
      }
    }
